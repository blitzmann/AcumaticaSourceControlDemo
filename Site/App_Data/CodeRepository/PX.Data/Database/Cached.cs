...
namespace PX.Data
{
	...
	public class PXView
	{
		...
		public sealed class Dummy : PXView
		{
			...
			public static Dummy For<TTable>(PXGraph graph) where TTable : class, IBqlTable, new() => new Dummy(graph, new Select<TTable>(), graph.Caches<TTable>().Current.AsSingleEnumerable().ToList());
		}
				...
				else
				{
					...
					foreach (string key in defsorts)
					{
						if (!CompareIgnoreCase.IsInList(cols, key)
                            && (!identitysort || Cache.Keys.Count != 1 || !CompareIgnoreCase.IsInList(cols, Cache.Keys[0])))
						{
							cols.Add(key);
						}
					}
		...
		/// <summary>
		/// Revomes a resultset from the cache of the results of BQL statement execution.
		/// </summary>
...
//			if (SelectQueries.IsCommandMutable && queryKey.CommandText == null && queryKey.BadParamsQueryNotExecuted) 
			...
			)
		{
		    ...
		    key.CacheType = Cache.GetItemType();
