	...
	public class PXSelectorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber, IPXFieldSelectingSubscriber, IPXDependsOnFields
	{
		...
		internal static object GetItem(PXCache cache, PXSelectorAttribute attr, object data, object key, bool unconditionally)
		{
		    ...
		    if (row == null && (key == null || key.GetType() == cache.GetFieldType(attr._FieldName)))
			{
				...
				if (attr._CacheGlobal && key != null && view.Cache.GetItemType().IsInstanceOfType(row) && view.Cache.GetStatus(row) == PXEntryStatus.Notchanged && !PXDatabase.ReadDeleted && attr.CanCacheGlobal(view.Cache))
				{
	    ...
	    {
			if (_CacheGlobal && dict != null && foreignItem != null && foreignCache.GetItemType().IsInstanceOfType(foreignItem)&&foreignCache.GetStatus(foreignItem) == PXEntryStatus.Notchanged && !PXDatabase.ReadDeleted && CanCacheGlobal(foreignCache))
			{
