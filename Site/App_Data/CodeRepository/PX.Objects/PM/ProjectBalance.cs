...
namespace PX.Objects.PM
{
	public class ProjectBalance
	{
			...
			if (tran.TaskID != null)
			{
				...
				else
				{
					if (!debitcreditcancelout)
					{
						...
						if (acc.Type == AccountType.Income || acc.Type == AccountType.Liability)
							list.Add(Calculate(project, tran, ag, acc.Type, -1 * invert));
						else
							list.Add(Calculate(project, tran, ag, acc.Type, 1 * invert));
					}
				}
				...
				if (offsetAcc != null && offsetAg != null && offsetAcc.AccountID != null && offsetAg.GroupID != null && !debitcreditcancelout)
				{
					...
					if (offsetAcc.Type == AccountType.Income || offsetAcc.Type == AccountType.Liability)
						list.Add(Calculate(project, tran, offsetAg, offsetAcc.Type, 1 * offsetInvert));
					else
						list.Add(Calculate(project, tran, offsetAg, offsetAcc.Type, -1 * offsetInvert));
				}
			}
		...
		public virtual Result Calculate(PMProject project, PMTran pmt, PMAccountGroup ag, string accountType, int mult)
		{
			...
			{				
				...
				decimal curyAmt = mult * pmt.ProjectCuryAmount.GetValueOrDefault();
