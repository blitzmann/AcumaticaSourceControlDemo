	...
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
		...
		public virtual void InsertTransactionsInInvoice(PMProject project, List<BillingData> unbilled, int mult)
		{
			foreach (BillingData data in unbilled)
			{
				...
				if (data.Tran.IsPrepayment != true && data.Tran.AccountGroupID != null)
					InvoiceEntry.SubtractAmountToInvoice(newTran, data.Tran.AccountGroupID, (int)ARDocType.SignAmount(newTran.TranType).GetValueOrDefault(1));
			...
			if (rule.FullDetail)
			{
				foreach (PMTran tran in billingBase)
				{
					...
					if (project.CuryID == project.BillingCuryID)
					{
						...
						line.CuryAmount = tran.Billable == true ? line.CuryBillableAmount : 0;						
			...
			else
			{
				...
				foreach (Group group in grouping.BreakIntoGroups(billingBase))
				{
					...
					if(project.CuryID == project.BillingCuryID)
					{
						...
						line.CuryAmount = line.CuryBillableAmount;						
						line.CuryRetainage = line.CuryAmount * line.RetainagePct;
						line.CuryUnitPrice = line.Qty != 0 ? line.CuryAmount / line.Qty : 0;
					}
					else
					{
		...
		public virtual List<BillingData> BillFixPriceTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			foreach (PXResult<PMRevenueBudget, PMAccountGroup> res in select.Select(task.TaskID))
			{
				...
				{
					unbilledAmount = (project.CuryID == project.BillingCuryID ? line.CuryRevisedAmount.GetValueOrDefault() : line.RevisedAmount.GetValueOrDefault()) * ratio.Value;
				}
				else
				{
		...
		public virtual PMTran ReverseTranGL(PMTran tran)
		{
			...
			if (offsetAccount != null && offsetAccount.AccountGroupID != null)
			{
				...
				rvrs.Released = null;
				rvrs.RemainderOfTranID = null;
				rvrs.DuplicateOfTranID = null;
			...
			else
			{
				...
				rvrs.RemainderOfTranID = null;
				rvrs.DuplicateOfTranID = null;
				rvrs.ProjectCuryAmount *= -1;
				rvrs.TranCuryAmount *= -1;
				rvrs.TranCuryAmountCopy = null;
				rvrs.Amount *= -1;
				rvrs.Qty *= -1;
				rvrs.BillableQty *= -1;
				return rvrs;
		...
		public virtual IList<PMTran> ReverseTranNonGL(PMTran tran)
		{
			...
			if (tran.OffsetAccountGroupID != null)
			{
				...
				credit.ProjectCuryAmount = tran.ProjectCuryAmount;
				credit.TranCuryAmount = tran.TranCuryAmount;
		...
		public virtual void InitializeRatios(int? projectID)
		{
			if (ratios == null)
			{
				var selectCostBudgetWithProduction = new PXSelectGroupBy<PMCostBudget, Where<PMCostBudget.isProduction, Equal<True>,
						...
						Sum<PMCostBudget.curyActualAmount>>>>>>>(this);
				...
				foreach (PMCostBudget total in selectCostBudgetWithProduction.Select(projectID))
				{
					...
					{
						ratio = decimal.Round(total.CuryActualAmount.GetValueOrDefault() / total.CuryRevisedAmount.GetValueOrDefault(), 2);
					}
