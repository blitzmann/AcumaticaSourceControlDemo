...
namespace PX.Objects.PM
{
    ...
    public class ProjectEntry : PXGraph<ProjectEntry, PMProject>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		{
			var graph = CreateInstance<PO.POOrderEntry>();
			...
			throw new PXRedirectRequiredException(graph, true, Messages.CreatePurchaseOrder) { Mode = PXBaseRedirectException.WindowMode.Same };
		}
		...
		protected virtual void _(Events.RowSelected<PMProject> e)
		{
			...
			if (e.Row != null)
			{
				...
				Sum<PMTaskTotal.curyAsset, Sum<PMTaskTotal.curyLiability, Sum<PMTaskTotal.curyIncome, Sum<PMTaskTotal.curyExpense>>>>>>>>>>(this);
				...
				if (tt != null)
				{
					e.Row.CuryAsset = tt.CuryAsset;
					e.Row.CuryLiability = tt.CuryLiability;
					e.Row.CuryIncome = tt.CuryIncome;
					e.Row.CuryExpense = tt.CuryExpense;
				}
		...
		protected virtual void _(Events.FieldSelecting<PMRevenueBudget, PMRevenueBudget.completedPct> e)
		{
			if (e.Row != null && AutoCompleteRevenueBudget())
			{
				...
				if (resultset.Count > 0)
				{
					...
					{
						revisedTotal += cost.CuryRevisedAmount.GetValueOrDefault();
						actualTotal += cost.CuryActualAmount.GetValueOrDefault();
					}
		...
		protected virtual void _(Events.FieldSelecting<PMRevenueBudget, PMRevenueBudget.curyAmountToInvoice> e)
		{
			if (e.Row != null)
			{
				...
				{
					decimal budgetedAmount = e.Row.CuryRevisedAmount.GetValueOrDefault();
					decimal result = (budgetedAmount * e.Row.CompletedPct.GetValueOrDefault() / 100m) - e.Row.CuryInvoicedAmount.GetValueOrDefault();

					PXFieldState fieldState = PXDecimalState.CreateInstance(result, (short)2, nameof(PMRevenueBudget.CuryAmountToInvoice), false, 0, Decimal.MinValue, Decimal.MaxValue);
					fieldState.Enabled = false;
		...
		public virtual void ApplyChangeOrderBudget(PMChangeOrderBudget changeBudget, PMChangeOrder order)
		{
			PMBudget updatedBudget = null;
			if (changeBudget.Type == GL.AccountType.Income)
			{
			   ...
			   r.AccountGroupID == changeBudget.AccountGroupID && r.InventoryID == changeBudget.InventoryID && r.CostCodeID == changeBudget.CostCodeID);
				...
				if (budget != null)
				{
					...
					budget.CuryChangeOrderAmount = budget.CuryChangeOrderAmount.GetValueOrDefault() + changeBudget.Amount.GetValueOrDefault();
					budget.RevisedQty = budget.RevisedQty.GetValueOrDefault() + changeBudget.Qty.GetValueOrDefault();
					budget.CuryRevisedAmount = budget.CuryRevisedAmount.GetValueOrDefault() + changeBudget.Amount.GetValueOrDefault();

					updatedBudget = RevenueBudget.Update(budget);					
				}
				else
				{
					...
					budget.CuryUnitRate = changeBudget.Rate;
					budget.ChangeOrderQty = changeBudget.Qty;
					budget.RevisedQty = changeBudget.Qty;
					budget.CuryChangeOrderAmount = changeBudget.Amount;
					budget.CuryRevisedAmount = changeBudget.Amount;
			...
			else if (changeBudget.Type == GL.AccountType.Expense)
			{
			   ...
			   r.AccountGroupID == changeBudget.AccountGroupID && r.InventoryID == changeBudget.InventoryID && r.CostCodeID == changeBudget.CostCodeID);
				...
				if (budget != null)
				{
					...
					updatedBudget = CostBudget.Update(budget);
				}
				else
				{
			...
			if (updatedBudget != null)
			{
				if (PXAccess.FeatureInstalled<FeaturesSet.budgetForecast>())
				{
					...
					if (finPeriod != null)
					{
						...
						forecast.ProjectID = updatedBudget.ProjectID;
						forecast.ProjectTaskID = updatedBudget.ProjectTaskID;
						forecast.AccountGroupID = updatedBudget.AccountGroupID;
						forecast.InventoryID = updatedBudget.InventoryID;
						forecast.CostCodeID = updatedBudget.CostCodeID;
						forecast.PeriodID = finPeriod.FinPeriodID;

						forecast = ForecastHistory.Insert(forecast);

						forecast.ChangeOrderQty += changeBudget.Qty.GetValueOrDefault();
						forecast.CuryChangeOrderAmount += changeBudget.Amount.GetValueOrDefault();
					}
				}
			}
		}
