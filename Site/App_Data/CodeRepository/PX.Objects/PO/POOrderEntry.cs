...
namespace PX.Objects.PO
{
	...
	public class POOrderEntry : PXGraph<POOrderEntry, POOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		{
			POLine row = (POLine)e.Row;
			foreach (PXResult<SOLineSplit3, INItemPlan> r in this.FixedDemand.View.SelectMultiBound(new object[] { e.Row }))
			{
		...
		public virtual void VerifyExchangeRateExistsForProject(int? newProjectID)
		{
			...
			if (!IsCostCommitmentTrackingEnabled())
				return;

			PMProject project;
			if (ProjectDefaultAttribute.IsProject(this, newProjectID, out project) && Document.Current != null)
			{
				var curySettings = PMCommitmentAttribute.GetCurrencySettings(this, newProjectID, Document.Current.CuryID);
				...
				if (!curySettings.UseBaseCurrency && curySettings.ConverionRequired)
				{
					...
					var rate = currencyService.GetRate(curySettings.FromCuryID, curySettings.ToCuryID, curySettings.RateTypeID, Document.Current.OrderDate.GetValueOrDefault(DateTime.Now));

					if (rate == null)
					{
						var ex = new PXSetPropertyException(PM.Messages.ConversionRateNotDefinedForCommitment, curySettings.FromCuryID, curySettings.ToCuryID, curySettings.RateTypeID, Document.Current.OrderDate.GetValueOrDefault(DateTime.Now));
						ex.ErrorValue = project.ContractCD;
		...
		protected virtual bool IsCostCommitmentTrackingEnabled()
		{
			if (!PXAccess.FeatureInstalled<FeaturesSet.projectModule>())
				return false;
		...
		{
			POOrderEntry.ClearUnused(poLine, poLine.LineType);
		...
		public virtual void SetBehaviorBasedOnLines()
		{
			if (Document.Current != null && PXAccess.FeatureInstalled<FeaturesSet.changeOrder>())
			{
				...
				var select = new PXSelect<PMChangeOrderLine,
					Where<PMChangeOrderLine.pOOrderType, Equal<Current<POOrder.orderType>>,
					And<PMChangeOrderLine.pOOrderNbr, Equal<Current<POOrder.orderNbr>>>>>(this);
