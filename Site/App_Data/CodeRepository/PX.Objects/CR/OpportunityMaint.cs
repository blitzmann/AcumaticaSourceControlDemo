	...
	public class OpportunityMaint : PXGraph<OpportunityMaint>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public virtual IEnumerable CreateQuote(PXAdapter adapter)
		{
			foreach (CROpportunity opportunity in adapter.Get())
			{
				...
				if (QuoteInfo.Current.QuoteType == CRQuoteTypeAttribute.Project && !PXAccess.FeatureInstalled<FeaturesSet.projectMultiCurrency>() && OpportunityCurrent.Current.CuryID != (Accessinfo.BaseCuryID ?? new PXSetup<PX.Objects.GL.Company>(this).Current?.BaseCuryID))
				{
        ...
        protected virtual void CreateQuotesFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
	    {
			...
			if (row.QuoteType == CRQuoteTypeAttribute.Project && !PXAccess.FeatureInstalled<FeaturesSet.projectMultiCurrency>() && OpportunityCurrent.Current.CuryID != (Accessinfo.BaseCuryID ?? new PXSetup<PX.Objects.GL.Company>(this).Current?.BaseCuryID))
			{
		...
		public class ContactAddress : ContactAddressGraph<OpportunityMaint>
		{
			...
			protected override CRShippingContact GetCurrentShippingContact()
			{
				var contact = Base.Shipping_Contact.SelectSingle();
				return contact;
			}
			protected override CRShippingAddress GetCurrentShippingAddress()
			{
			...
			protected override CRShippingContact GetEtalonShippingContact()
			{
			...
			protected override CRShippingAddress GetEtalonShippingAddress()
			{
