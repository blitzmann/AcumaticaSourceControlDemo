	...
	public class JournalEntry : PXGraph<JournalEntry, Batch>, PXImportAttribute.IPXPrepareItems, PX.Objects.GL.IVoucherEntry
	{
        ...
        public class JournalEntryDocumentExtension : DocumentWithLinesGraphExtension<JournalEntry>
	    {
	            ...
	            DocumentLine line)
	        {
	            ...
	            if (!e.Cache.ObjectsEqual<Document.headerDocDate>(e.Row, e.OldRow) && Base.Mode != Modes.Reclassification)
	            {
			...
			if (BatchModule.Current != null && BatchModule.Current.Module != GL.BatchModule.GL)
			{
				...
				foreach (PXResult res in resultset)
				{
					...
					if (PXAccess.FeatureInstalled<FeaturesSet.projectMultiCurrency>())
					{
						...
						if (project.CuryID == ledger.Current.BaseCuryID)
						{
						...
						else if (project.CuryID == BatchModule.Current.CuryID)
						{
							...
							projectCuryInfo.CuryRateTypeID = project.RateTypeID ?? CMSetup.Current.PMRateTypeDflt;
					...
					else
					{
						...
						pmt.ProjectCuryAmount = pmt.Amount;
						pmt.TranCuryID = ledger.Current.BaseCuryID;
						pmt.ProjectCuryID = ledger.Current.BaseCuryID;

						if (BatchModule.Current.CuryID != ledger.Current.BaseCuryID)
						{
							CurrencyInfo baseCuryInfo = new CurrencyInfo();
							baseCuryInfo.ModuleCode = GL.BatchModule.PM;
							baseCuryInfo.BaseCuryID = ledger.Current.BaseCuryID;
							baseCuryInfo.CuryID = ledger.Current.BaseCuryID;
							baseCuryInfo.CuryRateTypeID = null;
							baseCuryInfo.CuryEffDate = tran.TranDate;
							baseCuryInfo.CuryRate = 1;
							baseCuryInfo.RecipRate = 1;
							baseCuryInfo = currencyinfo.Insert(baseCuryInfo);
					...
					ProjectBalance.Result balance = pb.Calculate(project, pmt, ag, acc.Type, sign);
					...
					if (origTran != null && origTran.AccountID == tran.AccountID && origTran.Amount > tran.CreditAmt)
					{
						...
						{
							decimal val;
							PXCurrencyAttribute.CuryConvCury(BatchModule.Cache, projectCuryInfo, origTran.Amount.GetValueOrDefault() - tran.CreditAmt.GetValueOrDefault(), out val);
		...
		public virtual void IntegrityCheckProc(Ledger ledger, string startingPeriod)
		{
											...
											.Select(this, startingPeriod))
			{
				IEnumerable<PXResult<GLTran, CATran, Account, Ledger>> glHistoryUpdateData =
					...
					.Select(period.FinPeriodID, startingPeriod, period.FinPeriodID, ledger.LedgerID)
			...
			using (PXTransactionScope ts = new PXTransactionScope())
            {
				PXUpdateJoin<
				   ...
				   GLHistory,
					   ...
					   LeftJoin<OrganizationFinPeriodExt,
							 On<OrganizationFinPeriodExt.finPeriodID, Equal<Required<OrganizationFinPeriodExt.masterFinPeriodID>>,
					   ...
					   LeftJoin<GLHistoryByPeriod,
						   On<GLHistoryByPeriod.ledgerID, Equal<GLHistory.ledgerID>,
							   ...
							   And<GLHistoryByPeriod.finPeriodID, Equal<OrganizationFinPeriodExt.prevFinPeriodID>>>>>>,
					   LeftJoin<AcctHist2,
						   On<GLHistoryByPeriod.branchID, Equal<AcctHist2.branchID>,
							   And<GLHistoryByPeriod.ledgerID, Equal<AcctHist2.ledgerID>,
							   And<GLHistoryByPeriod.accountID, Equal<AcctHist2.accountID>,
							   And<GLHistoryByPeriod.subID, Equal<AcctHist2.subID>,
							   And<GLHistoryByPeriod.lastActivityPeriod, Equal<AcctHist2.finPeriodID>,
							   And<Where<Account.type, Equal<AccountType.asset>,
										   Or2<Where<Account.type, Equal<AccountType.liability>,
												   And<Account.accountID, NotEqual<GLSetup.ytdNetIncAccountID>>>,
										   Or<FinPeriod.finYear, Equal<Substring<GLHistoryByPeriod.lastActivityPeriod, int1, int4>>>>>>>>>>>>>>>>>>,
				   ...
				   .Update(this, startingPeriod, ledger.LedgerID, startingPeriod);
				...
				PXUpdateJoin<
					...
					GLHistory,
						...
						LeftJoin<GLHistoryByPeriod,
							On<GLHistoryByPeriod.ledgerID, Equal<GLHistory.ledgerID>,
								...
								And<GLHistoryByPeriod.accountID, Equal<GLHistory.accountID>,
								And<GLHistoryByPeriod.subID, Equal<GLHistory.subID>,
								And<GLHistoryByPeriod.finPeriodID, Equal<Required<FinPeriod.masterFinPeriodID>>>>>>>,
						LeftJoin<AcctHist2,
							On<GLHistoryByPeriod.branchID, Equal<AcctHist2.branchID>,
								And<GLHistoryByPeriod.ledgerID, Equal<AcctHist2.ledgerID>,
								And<GLHistoryByPeriod.accountID, Equal<AcctHist2.accountID>,
					...
					.Update(this, prevStartPeriod, ledger.LedgerID, startingPeriod);				
			...
			if (hist != null && hist.AccountID == reacct && FinPeriodUtils.PeriodInYear(hist.FinPeriodID) == "01"  && e.TranStatus == PXTranStatus.Open)
			{
				...
				{					
					...
					if (hist.YearClosed == true)
					{
					  ...
					  PXUpdateJoin<
							Set<AcctHist.yearClosed, Switch<Case<Where<Substring<AcctHist.finPeriodID, int5, int2>, Equal<string01>>, True>, False>,
							Set<AcctHist.curyFinBegBalance, Add<AcctHist.curyFinBegBalance, IsNull<AcctHist2.curyFinYtdBalance, decimal0>>,
							Set<AcctHist.curyFinYtdBalance, Add<AcctHist.curyFinYtdBalance, IsNull<AcctHist2.curyFinYtdBalance, decimal0>>,
							Set<AcctHist.curyTranBegBalance, Add<AcctHist.curyTranBegBalance, IsNull<AcctHist2.curyTranYtdBalance, decimal0>>,
							Set<AcctHist.curyTranYtdBalance, Add<AcctHist.curyTranYtdBalance, IsNull<AcctHist2.curyTranYtdBalance, decimal0>>,
						...
						.Update(sender.Graph, niacct, prevFinPeriod?.FinPeriodID, prevFinPeriod?.FinPeriodID, reacct, hist.SubID, hist.LedgerID, hist.BranchID, hist.FinPeriodID);
				}
			}
