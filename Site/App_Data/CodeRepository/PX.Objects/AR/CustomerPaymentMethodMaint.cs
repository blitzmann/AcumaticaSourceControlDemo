	...
	public class CustomerPaymentMethodMaint : PXGraph<CustomerPaymentMethodMaint, CustomerPaymentMethod>
	{
		...
		protected virtual void CustomerPaymentMethod_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				if (payment != null)
				{
					if (CCProcessingHelper.IsTokenizedPaymentMethod(this, row.PMInstanceID)
		...
		{
			string EntityCannotBeDeletedBecauseOfOneRefRecord = "{0} cannot be deleted because it is referenced in the following record: {1}.";

			string parentRecordInfo = GetRecordInfo(row);
			string childRecordInfo = GetRecordInfo(payment);

			string msg = PXLocalizer.LocalizeFormat(EntityCannotBeDeletedBecauseOfOneRefRecord, parentRecordInfo, childRecordInfo);
			return msg;
		}
