		...
		protected virtual IEnumerable ardocumentlist()
		{
			PXSelectBase<BalancedARDocument> selectDocumentsCommand = new PXSelectJoinGroupBy<BalancedARDocument,
				...
				Where2<
					...
					And2<Where<PaymentMethod.paymentType, NotEqual<PaymentMethodType.creditCard>,
	...
	public class ARReleaseProcess : PXGraph<ARReleaseProcess>
	{
			...
			if (invoicesNotCSL.Count() > 0)
			{
				foreach (SOInvoice soinvoice in invoicesNotCSL)
				{
					...
					if (soinvoice.IsCCCaptured == true)
					{
						...
						PXSelectBase<CCProcTran> paymentCCTran = new PXSelect<CCProcTran,
							Where<CCProcTran.docType, Equal<Required<CCProcTran.docType>>,
								And<CCProcTran.refNbr, Equal<Required<CCProcTran.refNbr>>>>,
							OrderBy<Desc<CCProcTran.tranNbr>>>(this);
						var result = paymentCCTran.Select(curPayment.DocType, curPayment.RefNbr);
						var paymentState = new PaymentState(result);
			...
			out PMRegister pmDoc)
		{
			...
			if (doc.Released != true)
			{
				...
				foreach (PXResult<ARTran, ARTax, Tax, DRDeferredCode, SO.SOOrderType> r in ARTran_TranType_RefNbr.Select((object)ardoc.DocType, ardoc.RefNbr))
				{
				    ...
				    ARTran_TranType_RefNbr.Cache.SetDefaultExt<ARTran.finPeriodID>(n);
				...
				if (proforma != null)
				{
					...
					foreach(PXResult<PMTran, PMProformaTransactLine> res2 in select.Select(proforma.RefNbr))
					{
						...
						else if (line.Option == PMProformaLine.option.HoldRemainder && line.CuryLineTotal < line.CuryBillableAmount && (string.IsNullOrEmpty(pmt.AllocationID) || pmt.Reverse == PMReverse.OnBilling) )
						{
							...
							if (n.LineType == SO.SOLineType.Inventory || n.LineType == SO.SOLineType.NonInventory)
							{
									...
									And<INTran.sOOrderType, Equal<Current<ARTran.sOOrderType>>, And<INTran.sOOrderNbr, Equal<Current<ARTran.sOOrderNbr>>, And<INTran.sOOrderLineNbr, Equal<Current<ARTran.sOOrderLineNbr>>>>>>>>.SelectMultiBound(this, new object[] { n }))
								{
									...
									if (intran.UpdateShippedNotInvoiced == true)
									{
										if (intran.Released != true)
										{
											throw new PXException(PXMessages.LocalizeFormatNoPrefixNLA(Messages.ShippedNotInvoicedINtranNotReleased, intran.RefNbr));
										}
				...
				if (_IsIntegrityCheck == false)
				{
					...
					{
						PM.RegisterEntry pmEntry = PXGraph.CreateInstance<PM.RegisterEntry>();
						pmEntry.ReverseCreditMemo(doc.RefNbr, creditMemoPMReversal);
						pmEntry.Actions.PressSave();
						pmDoc = pmEntry.Document.Current;
					}
					else if (nonglReverse.Count > 0 || billLaterNotAllocated.Count > 0 || reverseToNonBillable.Count > 0)
					{
						PM.RegisterEntry pmEntry = PXGraph.CreateInstance<PM.RegisterEntry>();
						pmEntry.NonGlBillLaterAndReverse(doc.RefNbr, doc.CuryID, nonglReverse, billLaterNotAllocated, reverseToNonBillable);
						pmEntry.Actions.PressSave();
						pmDoc = pmEntry.Document.Current;
					}
				}
		...
		protected virtual void ProcessSVATAdjustments(ARAdjust adj, ARRegister adjddoc, ARRegister adjgdoc)
		{
			if (PXAccess.FeatureInstalled<FeaturesSet.vATReporting>() && _IsIntegrityCheck == false)
			{
					...
					.SelectMultiBound(this, new object[] { adj }))
				{
					...
					if ((isPayment ? adjgdoc.CuryDocBal : adjddoc.CuryDocBal) == 0m)
					{
		...
		protected virtual void PerformBasicReleaseChecks(PXGraph selectGraph, ARRegister document)
		{
			...
			if (document.OrigModule == BatchModule.SO)
			{
				...
				if (CCProcTranHelper.IsCCPreAuthorizedSOInvoice(this, arInvoice))
				{
		...
		private void CheckOpenForReviewTrans(ARRegister doc)
		{
			...
			var trans = new PXSelect<CCProcTran, Where<CCProcTran.refNbr, Equal<Required<ARRegister.refNbr>>,
				And<CCProcTran.docType, Equal<Required<ARRegister.docType>>>>,
				OrderBy<Desc<CCProcTran.tranNbr>>>(this).Select(doc.RefNbr, doc.DocType).RowCast<CCProcTran>();
			var openReviewTran = CCProcTranHelper.FindOpenForReviewTran(trans.Cast<ICCPaymentTransaction>());
			if (openReviewTran != null)
			{
		...
		public virtual void IntegrityCheckProc(Customer cust, string startPeriod)
		{
			...
			using (new PXConnectionScope())
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					PXResultset<ARRegister> custdocs = PXSelect<
						ARRegister, 
						Where<ARRegister.customerID, Equal<Current<Customer.bAccountID>>, 
							And<ARRegister.released, Equal<True>, 
							And<Where<ARRegister.tranPeriodID, GreaterEqual<Required<ARRegister.tranPeriodID>>, 
								Or<ARRegister.closedTranPeriodID, GreaterEqual<Required<ARRegister.tranPeriodID>>>>>>>>
					...
					PXResultset<ARRegister> other = PXSelectJoinGroupBy<ARRegister,
						...
						InnerJoin<Standalone.ARRegister2, 
								On<Standalone.ARRegister2.docType, Equal<ARAdjust.adjdDocType>,
						...
						Aggregate<GroupBy<ARRegister.docType,
							...
							GroupBy<ARRegister.voided>>>>>>>>>>>>>>.Select(this, minPeriod, minPeriod, minPeriod, minPeriod, minPeriod);
