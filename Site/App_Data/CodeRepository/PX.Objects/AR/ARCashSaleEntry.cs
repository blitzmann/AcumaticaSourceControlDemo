...
namespace PX.Objects.AR
{
	public class ARCashSaleEntry : ARDataEntryGraph<ARCashSaleEntry, ARCashSale>, IGraphWithInitialization
	{
		...
		public PXSelect<CCProcTran, Where<CCProcTran.refNbr, Equal<Current<ARCashSale.refNbr>>, And<CCProcTran.docType, Equal<Current<ARCashSale.docType>>,
		...
		public class PaymentTransaction : PaymentTransactionGraph<ARCashSaleEntry, ARCashSale>
		{
			...
			protected override void RowSelected(Events.RowSelected<ARCashSale> e)
			{
				...
				bool canAuthorize = doc.Hold == false && docTypePayment && !(paymentState.isCCPreAuthorized || paymentState.isCCCaptured);
				bool canCapture = doc.Hold == false && docTypePayment && !paymentState.isCCCaptured;
				bool canVoid = doc.Hold == false && (doc.DocType == ARDocType.CashReturn && (paymentState.isCCCaptured || paymentState.isCCPreAuthorized)) ||
							   (paymentState.isCCPreAuthorized && docTypePayment);
				bool canCredit = doc.Hold == false && doc.DocType == ARDocType.CashReturn && !paymentState.isCCRefunded;
				bool canValidate = doc.Hold == false && docTypePayment && paymentState.isOpenForReview;
				...
				else if (enableCCProcess && (paymentState.isCCPreAuthorized || paymentState.isCCCaptured))
				{
					PXUIFieldAttribute.SetEnabled(cache, doc, false);
				...
				else
				{
					...
					PXUIFieldAttribute.SetEnabled<ARCashSale.refTranExtNbr>(cache, doc, enableCCProcess && ((doc.DocType == ARDocType.CashReturn) && !paymentState.isCCRefunded));
					PXUIFieldAttribute.SetEnabled<ARPayment.pMInstanceID>(cache, doc, isPMInstanceRequired);
					cache.AllowDelete = !CCProcTranHelper.HasCCTransactions(Base.ccProcTran);
				}

				bool enableVoidCheck = doc.Released == true && docTypePayment && doc.Voided == false;
				bool isCCStateClear = !(paymentState.isCCCaptured || paymentState.isCCPreAuthorized) || paymentState.isCCVoided || paymentState.isCCRefunded;

				if (doc.Released == false && !enableVoidCheck && docTypePayment && doc.Voided == false)
				{
					if (CCProcTranHelper.HasCCTransactions(Base.ccProcTran) && (isCCStateClear || (paymentState.isCCPreAuthorized && paymentState.isCCVoidingAttempted)))
				...
				if (enableCCProcess && (ARSetup.Current.IntegratedCCProcessing ?? false))
				{
					if (doc.Released == false)
					{
						bool releaseActionEnabled = doc.Hold == false &&
													doc.OpenDoc == true &&
												   (doc.DocType == ARDocType.CashReturn ? paymentState.isCCRefunded : paymentState.isCCCaptured);
