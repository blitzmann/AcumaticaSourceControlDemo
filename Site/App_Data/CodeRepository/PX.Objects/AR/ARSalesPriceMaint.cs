...
namespace PX.Objects.AR
{
	public class ARSalesPriceMaint : PXGraph<ARSalesPriceMaint>
	{
		...
		public virtual SalesPriceItem FindSalesPrice(PXCache sender, string custPriceClass, int? customerID, int? inventoryID, int? siteID, string baseCuryID, string curyID, decimal? quantity, string UOM, DateTime date, bool isFairValue)
		{
			...
			if (!priceListExists) return isFairValue ? null : SelectDefaultItemPrice(sender, inventoryID, baseCuryID);
		...
		private class SalesPriceSelect
		{
			...
			protected ARSalesPrice SelectCustomerPrice(decimal quantity, params object[] additionalParams)
			{
				return SelectCommand
					.SelectSingle(
						new object[] { InventoryID, IsFairValue, CustomerID, CustPriceClass, CustomerID, CustPriceClass, CuryID, SiteID, quantity, Date, Date, Date, Date }
			...
			protected ARSalesPrice SelectBasePrice(decimal quantity, params object[] additionalParams)
			{
				return SelectCommand
					.SelectSingle(
						new object[] { InventoryID, IsFairValue, CustomerID, CustPriceClass, null, null, CuryID, SiteID, quantity, Date, Date, Date, Date }
							.Concat(additionalParams)
							.ToArray());
			}
