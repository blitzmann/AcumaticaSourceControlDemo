...
namespace PX.Objects.FA
{
	...
	public class AssetProcess : PXGraph<AssetProcess>
	{
		...
		public static DocumentList<FARegister> SplitAsset(FixedAsset asset, DateTime? splitDate, string splitPeriodID, bool deprBeforeSplit, Dictionary<FixedAsset, decimal> splits)
		{
			...
			foreach (KeyValuePair<FixedAsset, decimal> split in splits)
			{
				foreach (FABookBalance sbal in PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Required<FixedAsset.assetID>>>>.Select(docgraph, split.Key.AssetID))
				{
					...
					{
						if ((!string.IsNullOrEmpty(bal.LastDeprPeriod) || item.Suspended == true) &&
			...
			foreach (FABookBalance item in books)
			{
				...
				try
				{
					using (PXTransactionScope ts = new PXTransactionScope())
					{
						...
						foreach (PXResult<FABookHistory, FixedAsset, Branch, FABook, FABookPeriod> res in PXSelectJoin<
							FABookHistory, 
		...
		{
			FADetails faDetails = PXSelect<FADetails, Where<FADetails.assetID, Equal<Required<FADetails.assetID>>>>.Select(graph, assetID);
			if (faDetails != null &&
				PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Required<FABookBalance.assetID>>>>
					.Select(graph, faDetails.AssetID)
					.RowCast<FABookBalance>()
					.All(bookBalance => bookBalance.Status == FixedAssetStatus.FullyDepreciated))
			{
				faDetails.Status = FixedAssetStatus.FullyDepreciated;
				graph.Caches<FADetails>().Update(faDetails);
			}
		}
