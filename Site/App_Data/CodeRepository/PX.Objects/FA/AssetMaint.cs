	...
	public class AssetMaint : PXGraph<AssetMaint, FixedAsset>, IGraphWithInitialization
	{
		...
		protected virtual void GLTranFilter_TranDate_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			FABookPeriod currentPeriod =
				PXSelectJoin<
					...
					Where<FABookBalance.updateGL, Equal<True>,
						And<FABookPeriod.endDate, NotEqual<FABookPeriod.startDate>,
						And<FABookPeriod.organizationID, Equal<Required<FABookPeriod.organizationID>>>>>>.Select(this, PXAccess.GetParentOrganizationID(CurrentAsset.Current?.BranchID));
		...
		public override void Persist()
		{
			...
			if (asset != null && det.ReceiptDate != null)
			{
				...
				{
					FABook book = PXSelect<FABook, Where<FABook.updateGL, Equal<True>>>.SelectSingleBound(this, null);
					currentLocation.PeriodID = FABookPeriodRepository.GetFABookPeriodIDOfDate(det.ReceiptDate, book.BookID, asset.AssetID);
		...
		protected virtual void FixedAsset_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			bool isSplittedAsset = asset.SplittedFrom != null 
				|| SelectFrom<FixedAsset>.Where<FixedAsset.splittedFrom.IsEqual<@P.AsInt>>
					.View
					.Select(this, asset.AssetID)
					.Any();

			bool fixedAssetAccountUsed = SelectFrom<FATran>
			...
			bool accumulatedDepreciationAccounttUsed = SelectFrom<FATran>
				.Where<FATran.assetID.IsEqual<@P.AsInt>
					.And<FATran.released.IsEqual<True>>
					.And<FATran.batchNbr.IsNotNull>
			...
			bool haveSplittedDepreciation = SelectFrom<FATran>
				.InnerJoin<FABook>
			...
			bool fixedAssetAccountEnabled = !(fixedAssetAccountUsed || isSplittedAsset);
			bool accumulatedDepreciationAccountEnabled = !(accumulatedDepreciationAccounttUsed || (isSplittedAsset && haveSplittedDepreciation));
			...
			where ParentField : IBqlField
		{
			...
			Select<BqlPlaceholder1, 
				Where<BqlPlaceholder2, Equal<Required<BqlPlaceholder3>>>>>
				.Replace<BqlPlaceholder1>(balType)
				.Replace<BqlPlaceholder2>(balanceCache.GetBqlField<FixedAsset.assetID>())
				.Replace<BqlPlaceholder3>(sourceCache.GetBqlField<FixedAsset.assetID>())
				.ToCommand();
