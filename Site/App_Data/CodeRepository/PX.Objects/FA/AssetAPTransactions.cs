	...
	public class AssetGLTransactions : PXGraph<AssetGLTransactions>
	{
		...
		protected virtual void _(Events.FieldVerifying<FATran.tranDate> e)
		{
			...
			if(string.CompareOrdinal(receiptDatePeriodID, deprFromDatePeriodID) < 0 && string.CompareOrdinal(tranPeriodID, receiptDatePeriodID) < 0)
			{
				throw new PXSetPropertyException(
					...
					nameof(FATran.receiptDate),
					FABookPeriodIDAttribute.FormatForError(receiptDatePeriodID));
			}
			if (string.CompareOrdinal(receiptDatePeriodID, deprFromDatePeriodID) >= 0 && string.CompareOrdinal(tranPeriodID, deprFromDatePeriodID) < 0)
			{
				throw new PXSetPropertyException(
					Messages.IncorrectPurchasingPeriod,
					FABookPeriodIDAttribute.FormatForError(tranPeriodID),
					nameof(FATran.deprFromDate),
					FABookPeriodIDAttribute.FormatForError(deprFromDatePeriodID));
			}
		}
		...
		public override void Persist()
		{
			...
			foreach (FATran tran in ptrans)
			{
				...
				{
					// Mapped book period
					FABookPeriod targetFABookPeriod = FABookPeriodRepository.GetMappedFABookPeriodByBranches(
						bal.BookID,
						gltran.BranchID,
						gltran.FinPeriodID,
						tran.BranchID);
					...
					OrganizationFinPeriod glperiod = FinPeriodUtils
						.GetNearestOpenOrganizationFinPeriodInSubledger<OrganizationFinPeriod.fAClosed>(targetFABookPeriod);
