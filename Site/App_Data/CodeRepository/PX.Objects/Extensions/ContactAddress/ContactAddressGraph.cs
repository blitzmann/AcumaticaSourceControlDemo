	...
	public abstract class ContactAddressGraph<TGraph> : PXGraphExtension<TGraph> where TGraph : PXGraph
	{
		...
		protected virtual void _(Events.FieldUpdated<Document, Document.allowOverrideShippingContactAddress> e)
		{
			...
			if (cacheShippingContact != null)
			{
				...
				CRShippingContact crShippingContact = cacheShippingContact.Current as CRShippingContact;
			...
			if (cacheShippingAddress != null)
			{
				...
				CRShippingAddress crShippingAddress = GetCurrentShippingAddress();
		...
		protected virtual void DefaultRecords(Document row, ChangedData changedForContactInfo, ChangedData changedForShippingInfo)
		{
			...
			bool needAskFromContactAddress = row.AllowOverrideContactAddress == true &&
				                             !IsDefaultContactAdress() &&
											 !IsContactAddressNoChanged(changedForContactInfo);
			bool needAskFromShippingContactAddress = row.AllowOverrideShippingContactAddress == true &&
													 !IsDefaultShippingContactAdress() &&
													 !IsShippingContactAddressNoChanged(changedForShippingInfo);
			...
			if (row.LocationID == null && row.ContactID == null && row.BAccountID == null)
			{
				...
				if (row.AllowOverrideShippingContactAddress == false)
				{
					...
					cache.SetValue<Document.allowOverrideShippingContactAddress>(row, false);
					PXCache cacheContact = GetContactCache();
					CRContact contact = GetCurrentContact();
					cacheContact.RaiseRowUpdated(contact, contact);
					PXCache cacheAddress = GetAddressCache();
					CRAddress address = GetCurrentAddress();
		...
		protected abstract CRShippingContact GetCurrentShippingContact();
		protected abstract CRContact GetEtalonContact();
		protected abstract CRShippingContact GetEtalonShippingContact();
		protected abstract CRAddress GetCurrentAddress();
		protected abstract CRShippingAddress GetCurrentShippingAddress();
		protected abstract CRAddress GetEtalonAddress();
		protected abstract CRShippingAddress GetEtalonShippingAddress();
