...
namespace PX.Objects.Extensions.PaymentTransaction
{
		...
		where TPrimary : class, IBqlTable, new()
	{
		...
		public virtual IEnumerable SyncPaymentTransaction(PXAdapter adapter)
		{
			...
			var jObj = JObject.Parse(tranResponseStr);
			string tranId = jObj.SelectToken("transId")?.ToString();
			if (tranId == null)
			{
				throw new PXException(AR.Messages.ERR_AcceptHostedFormResponseFieldNotFound, "transId");
			}
		...
		private IEnumerable AuthorizeThroughForm(PXAdapter adapter)
		{
			...
			foreach (TPrimary doc in adapter.Get<TPrimary>())
			{
				...
				if (EnableMobileMode)
				{
					...
					appendParams.Add("TranType", CCTranType.AuthorizeOnly.ToString());
				...
				else
				{
					...
					try
					{
						paymentProcessing.ShowAcceptPaymentForm(V2.CCTranType.AuthorizeOnly, pDoc, SelectedProcessingCenter, SelectedBAccount);
					}
		...
		private IEnumerable CaptureThroughForm(PXAdapter adapter)
		{
			...
			foreach (TPrimary doc in adapter.Get<TPrimary>())
			{
				...
				else
				{
					...
					try
					{
						paymentProcessing.ShowAcceptPaymentForm(V2.CCTranType.AuthorizeAndCapture, pDoc, SelectedProcessingCenter, SelectedBAccount);
					}
					catch (PXPaymentRedirectException ex)
					{
						redirectEx = ex;
		...
		private void CheckPaymentTransaction(TPrimary doc)
		{
			...
			if (LockExists(doc))
			{
				...
				try
				{
					trans = retryUnsettledTran.Execute(() => paymentProcessing.GetUnsettledTransactions(SelectedProcessingCenter));
				}
			...
			{
				trans = paymentProcessing.GetUnsettledTransactions(SelectedProcessingCenter);
			}

			if (trans != null)
			{
				List<string> result = GetTransByDoc(pDoc, trans).OrderBy(i => i.SubmitTime).Select(i => i.TranID).ToList();
				if (result.Count > 0)
				{
		...
		public virtual void SyncPaymentTransactionById(TPrimary doc, List<string> tranIds)
		{
			...
			using (PXTransactionScope scope = new PXTransactionScope())
			{
				foreach (string tranId in tranIds)
				{
					...
					if (storedTran != null && CCTranStatusCode.GetCode(V1ProcessingDTOGenerator.ToV1(tranData.TranStatus)) == storedTran.TranStatus)
					{
						continue;
					}
		...
		protected virtual int? GetOrCreatePaymentProfilrByTran(V2.TransactionData tranData, ICCPayment pDoc)
		{
			...
			try
			{
				...
				ICCPaymentTransaction lastTran = CCProcTranHelper.FindCCLastSuccessfulTran(PaymentTransaction.Select().RowCast<PaymentTransactionDetail>());
				if (profile == null && lastTran != null && lastTran.TranType == CCTranTypeCode.Authorize)
				{
		...
		protected void CreateCustomerProcessingCenterRecord(V2.TranProfile input)
		{
			...
			customerProcessingCenterCache.ClearQueryCache();
		...
		{
			PXSelectBase<CustomerPaymentMethod> query = new PXSelectReadonly<CustomerPaymentMethod,Where<CustomerPaymentMethod.bAccountID,
				Equal<Required<CustomerPaymentMethod.bAccountID>>>,OrderBy<Desc<CustomerPaymentMethod.createdDateTime>>>(Base);

			IEnumerable<CustomerPaymentMethod> cpmRes = query.Select(SelectedBAccount).RowCast<CustomerPaymentMethod>();
		...
		{
			V2.TransactionData tranData = paymentProcessing.GetTransactionById(tranId, SelectedProcessingCenter);
			return tranData;
		}
		...
		private bool FindPreAuthorizing()
		{
			...
			return authTran == null ? false : true;
		}

		private bool TranHeldForReview()
		{
			ICCPaymentTransaction tranForReview = CCProcTranHelper.FindOpenForReviewTran(GetTrans());
			return tranForReview != null ? true : false;
		}
