...
namespace PX.Objects.AP
{
	...
	public class APPayBills : PXGraph<APPayBills>
	{
		...
		protected virtual IEnumerable aPExceptionsList()
		{
			...
			foreach (PXResult<APInvoice, CurrencyInfo, Vendor, APAdjust, APTran> res in invoices)
			{
				...
				if (locatedAdjust == null)
				{
					...
					yield return new PXResult<APAdjust, APInvoice, APTran>(APExceptionsList.Insert(adj), doc, tran);
				}
				else
				{
					yield return new PXResult<APAdjust, APInvoice, APTran>(locatedAdjust, doc, tran);
				}
			}
			PXView.StartRow = 0;
		...
		protected virtual IEnumerable apdocumentlist()
		{
			...
			{
				var result = (PXResult<APInvoice, APTran>)PXSelectJoin<APInvoice,
					LeftJoin<APTran, On<APInvoice.paymentsByLinesAllowed, Equal<True>,
						And<APTran.tranType, Equal<APInvoice.docType>,
						And<APTran.refNbr, Equal<APInvoice.refNbr>,
						And<APTran.lineNbr, Equal<Current<APAdjust.adjdLineNbr>>,
						And<APTran.curyTranBal, Greater<decimal0>>>>>>>,
					Where <APInvoice.docType, Equal<Current<APAdjust.adjdDocType>>, 
						And<APInvoice.refNbr, Equal<Current<APAdjust.adjdRefNbr>>>>>
					.SelectSingleBound(this, new object[]{ adj });
				APInvoice doc = result;
				APTran tran = result;
				adj.SeparateCheck = (adj.SeparateCheck ?? doc.SeparateCheck);

				yield return new PXResult<APAdjust, APInvoice, APTran>(adj, doc, tran);
								...
								ref totalRows))
			{
				...
				if (APDocumentList.Locate(adj) == null)
				{
					...
					yield return new PXResult<APAdjust, APInvoice, APTran>(APDocumentList.Insert(adj), doc, tran);
		...
		protected virtual void APAdjust_AdjdRefNbr_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
	        ...
	        {
			APAdjust adj = (APAdjust)e.Row;
                foreach (APInvoice res in
	                        PXSelect<APInvoice,
	                        Where<APInvoice.docType, Equal<Required<APInvoice.docType>>,
	                            And<APInvoice.refNbr, Equal<Required<APInvoice.refNbr>>>>>
	                    .Select(this, adj.AdjdDocType, adj.AdjdRefNbr))
					{
	                if (res.PaymentsByLinesAllowed != true)
					{
	                    adj.AdjdLineNbr = 0;
	                    sender.Update(adj);
					}
