		...
		protected virtual IEnumerable documents()
		{
			...
			else if (header.OrganizationID != null)
			{
				...
				sel.WhereAnd<Where<APRegister.branchID, In<Required<APRegister.branchID>>,
				  And<MatchWithBranch<APRegister.branchID>>>>();
			}
			...
			foreach (PXResult<APDocumentResult, Sub, APInvoice, APPayment> reg in sel.Select(branchIDs))
			{
				...
				if (byPeriod)
				{
					...
					//
					foreach (PXResult<APAdjust, APDocumentResult, Branch, FinPeriod> it in selectAdjusting.Select(res.DocType, res.RefNbr))
					{
						...
						if (isPrepayment)
						{
							if (adjustingDocument.Paying == true)
							{
								applicationAmount = adjustingDocument.DocType != APDocType.Refund
									? adjustment.AdjAmt + adjustment.AdjDiscAmt + adjustment.AdjWhTaxAmt + adjustment.SignedRGOLAmt
									: -adjustment.AdjAmt - adjustment.SignedRGOLAmt;
								curyApplicationAmount = adjustingDocument.DocType != APDocType.Refund
									? adjustment.CuryAdjdAmt + adjustment.CuryAdjdDiscAmt + adjustment.CuryAdjdWhTaxAmt
						...
						{
							decimal AdjDiscAmt = res.DocType == APDocType.QuickCheck ? 0m : (adjustment.AdjDiscAmt ?? 0m);
							decimal CuryAdjdDiscAmt = res.DocType == APDocType.QuickCheck ? 0m : (adjustment.CuryAdjdDiscAmt ?? 0m);

							applicationAmount = adjustment.AdjAmt + AdjDiscAmt + adjustment.AdjWhTaxAmt + adjustment.SignedRGOLAmt;
							curyApplicationAmount = adjustment.CuryAdjdAmt + CuryAdjdDiscAmt + adjustment.CuryAdjdWhTaxAmt;
							...
							if (adjustment.AdjgTranPeriodID != GetFilterMasterPeriodForAdjust(header, documentRelatedFilterFinPeriod))
							{
								...
								res.CuryBegBalance -= adjustment.CuryAdjdAmt + CuryAdjdDiscAmt + adjustment.CuryAdjdWhTaxAmt;
							}
						}

						#region APTurnover calculation

						aPTurnover = adjustment.AdjgTranPeriodID == GetFilterMasterPeriodForAdjust(header, documentRelatedFilterFinPeriod) &&
					...
					//
					foreach (PXResult<APAdjust, Branch, FinPeriod> row in selectAdjusted.Select(res.DocType, res.RefNbr))
					{
