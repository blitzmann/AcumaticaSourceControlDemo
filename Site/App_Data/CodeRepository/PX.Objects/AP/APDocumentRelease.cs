...
namespace PX.Objects.AP
{
	...
	public class APReleaseProcess : PXGraph<APReleaseProcess>
	{
			...
			out List<INRegister> inDocs)
		{
			...
			if (doc.Released != true && (!isPrebooking || doc.Prebooked != true))
			{
				...
				if (apdoc.DocType != APDocType.Prepayment)
				{
					...
					foreach (var group in apTranTaxes.AsEnumerable().GroupBy(row => (APTran)row))
					{
						APTran n = group.Key;
						...
						foreach (PXResult<APTran, APTax, Tax, DRDeferredCode, LandedCostCode, InventoryItem> r in group)
						{
							APTax x = r;
							Tax salestax = r;
							...
							!isMultipleInstallmentInvoice)
						{
							...
							maxRetainageTran = maxRetainageTran == null || maxRetainageTran.CuryRetainageBal < n.CuryRetainageBal ? n : maxRetainageTran;
							maxBalanceTran = maxBalanceTran == null || maxBalanceTran.CuryTranBal < n.CuryTranBal ? n : maxBalanceTran;
							...
							if (apdoc.IsChildRetainageDocument())
					{
								...
								if (origRetainageTran != null)
						{
									origRetainageTran.CuryRetainageBal -= n.CuryOrigTranAmt * apdoc.SignAmount;
									origRetainageTran = APTran_TranType_RefNbr.Update(origRetainageTran);

									if (origRetainageTran.CuryRetainageBal < 0m ||
										origRetainageTran.CuryRetainageBal > origRetainageTran.CuryOrigRetainageAmt)
							{
										throw new PXException(Messages.RetainageUnreleasedBalanceNegative);
									}
								}
						...
						doc.Prebooked != true)
					{
						...
						if (validateBalances.RetainageBalance.Cury != doc.CuryRetainageTotal)
						{
							...
							{
							maxBalanceTran.RetainageBal -= validateBalances.RetainageBalance.Base - doc.RetainageTotal;
							APTran_TranType_RefNbr.Update(maxBalanceTran);
						}
						...
						if (validateBalances.TranBalance.Cury != doc.CuryDocBal)
							...
							maxBalanceTran != null)
						{
							maxBalanceTran.TranBal -= validateBalances.TranBalance.Base - doc.DocBal;
							APTran_TranType_RefNbr.Update(maxBalanceTran);
							}
						}
			...
			Tax tax)
		{
			...
			LineBalances balances = includeTax
				? new LineBalances(
					new Amount(0m, 0m), 
					new Amount(aptax.CuryRetainedTaxAmt ?? 0m, aptax.RetainedTaxAmt ?? 0m),
					new Amount(aptax.CuryTaxAmt ?? 0m, aptax.TaxAmt ?? 0m))
				: new LineBalances(0m);
				...
				if (adj.AdjgDocType != adj.AdjdDocType || adj.AdjgRefNbr != adj.AdjdRefNbr)
				{
					...
					if (paymentRegister.PaymentsByLinesAllowed == true)
					{
						IEnumerable<PXResult<APTran>> transactions = PXSelect<APTran,
								Where<APTran.tranType, Equal<Current<APPayment.docType>>,
									And<APTran.refNbr, Equal<Current<APPayment.refNbr>>>>,
								OrderBy<Asc<APTran.lineNbr>>>
									.SelectMultiBound(this, new object[] {paymentRegister});
						if (curyAdjBal < 0)
		...
		public virtual void IntegrityCheckProc(Vendor vend, string startPeriod)
		{
			...
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					PXResultset<APRegister> venddocs = PXSelect<
						APRegister, 
						Where<APRegister.vendorID, Equal<Current<Vendor.bAccountID>>, 
							And2<
								...
								And<Where<APRegister.tranPeriodID, GreaterEqual<Required<APRegister.tranPeriodID>>, 
									Or<APRegister.closedTranPeriodID, GreaterEqual<Required<APRegister.tranPeriodID>>>>>>>>
					...
					PXResultset<APRegister> other = PXSelectJoinGroupBy<APRegister, 
						...
						InnerJoin<Standalone.APRegister2, 
							On<Standalone.APRegister2.docType, Equal<APAdjust.adjdDocType>,
						...
						Where<APRegister.vendorID, Equal<Current<Vendor.bAccountID>>, 
							...
							And2<Where<APRegister.released, Equal<True>,
									Or<APRegister.prebooked, Equal<True>>>,
							And<Where<APRegister.closedTranPeriodID, Less<Required<APRegister.closedTranPeriodID>>, 
						...
						foreach (PXResult<APPayment, CurrencyInfo, Currency, Vendor, CashAccount> res in APPayment_DocType_RefNbr.Select(doc.DocType, doc.RefNbr, doc.VendorID))
						{
							...
							while (doc.AdjCntr < OrigAdjCntr)
							{
								...
								foreach(PXResult<APAdjust, CurrencyInfo, Currency, APInvoice, APPayment, Standalone.APRegisterAlias> result in adjustments)
								{
									...
									if (adjddoc.DocType == APDocType.Prepayment && adjddoc.AdjCntr == 0 && !appliedToVoid)
									{
