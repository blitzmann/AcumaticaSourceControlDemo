...
namespace PX.Objects.AP
{
	...
	public class PaymentRefAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber 
	{
		...
		void IPXRowPersistingSubscriber.RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (oldNumber == null)
			{
				PaymentMethodAccount incdet = (PaymentMethodAccount)sender.Graph.Caches[typeof(PaymentMethodAccount)].Locate(det);
				oldNumber = incdet?.APLastRefNbr;
			}
			...
			if (string.IsNullOrEmpty(newNumber))
			{
				...
				{
					WebDialogResult result = check.IsReleaseCheckProcess == true
						? WebDialogResult.Yes
	...
	public class APPaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjTranPeriodID, WhereSelect> :
		PaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjTranPeriodID,
	    ...
	    where AdjTranPeriodID : IBqlField
	...
	public class PaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjTranPeriodID,
        ...
        where WhereSelect : IBqlWhere, new()
	{
		...
		{
			if (!sender.ObjectsEqual<AdjDate, AdjTranPeriodID, CashAccountID>(e.Row, e.OldRow))
			{
			    foreach (ChargeTable charge in this.View.SelectMulti())
			    {
                    ...
                    if (!sender.ObjectsEqual<AdjDate>(e.Row, e.OldRow))
			        {
			            View.Cache.SetDefaultExt<ChargeTranDate>(charge);
			        }

			        if (!sender.ObjectsEqual<AdjTranPeriodID>(e.Row, e.OldRow))
			        {
