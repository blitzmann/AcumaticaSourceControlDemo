	...
	public class APPaymentEntry : APDataEntryGraph<APPaymentEntry, APPayment>
	{
		...
		private void LoadInvoicesProc(bool LoadExistingOnly)
		{
			...
			try
			{
				...
				foreach (PXResult<APInvoice,APTran> rec in venddocs)
				{
		...
		public static PXResultset<APInvoice> GetVendDocs(APPayment currentAPPayment, PXGraph graph, APSetup currentAPSetup)
		{
			...
			var venddocs = new PXResultset<APInvoice, APTran>();
			foreach (PXResult<APInvoice, APTran> rec in
		...
		{
			var ret = new PXResultset<APAdjust, APInvoice, APTran>();
				...
				in Adjustments_Invoices.Select())
			{
				...
				ret.Add(new PXResult<APAdjust, APInvoice, APTran>(res, fullInvoice, tran));
			}
		...
		protected virtual IEnumerable adjustments_history()
		{
            ...
            foreach (PXResult<APAdjust, APInvoice, Standalone.APRegisterAlias, APTran> result in outgoingApplications.Select())
			{
                ...
                resultSet.Add(new PXResult<APAdjust, APInvoice, APTran>(result, fullInvoice, tran));
			}
            ...
            foreach (PXResult<APAdjust, APPayment, Standalone.APRegisterAlias, CurrencyInfo> result in incomingApplications.Select())
            {
                ...
                resultSet.Add(new PXResult<APAdjust, APPayment, APTran>(incomingApplication, appliedPayment, null));
            }
		...
		protected virtual void InitApplicationData(APAdjust adj)
		{
			try
				{
				foreach (PXResult<APInvoice, CurrencyInfo, APTran> res in PXSelectJoin<APInvoice,
					...
					LeftJoin<APTran, On<APInvoice.paymentsByLinesAllowed, Equal<True>,
						...
						And<APTran.lineNbr, Equal<Required<APAdjust.adjdLineNbr>>>>>>>>,
																		    Where<APInvoice.vendorID, Equal<Current<APPayment.vendorID>>,
																			  And<APInvoice.docType, Equal<Required<APInvoice.docType>>,
						And<APInvoice.refNbr, Equal<Required<APInvoice.refNbr>>>>>>
					.Select(this, adj.AdjdLineNbr, adj.AdjdDocType, adj.AdjdRefNbr))
					{
					APInvoice invoice = res;
		...
		private void CalcBalancesFromAdjustedDocument(object row, bool isCalcRGOL, bool DiscOnDiscDate)
		{
			...
			foreach (PXResult<APInvoice, APTran> res in APInvoice_VendorID_DocType_RefNbr.Select(adj.AdjdLineNbr, adj.VendorID, adj.AdjdDocType, adj.AdjdRefNbr))
		{
		...
		protected virtual void APPayment_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (doc.DocType == APPaymentType.Check && doc.Hold != true && doc.Released != true &&
				(
				...
				))
			{
				...
				{
					if (adj.AdjdDocType == APDocType.Prepayment && adj.CuryDocBal != 0m)
					{
						Adjustments.Cache.RaiseExceptionHandling<APAdjust.curyDocBal>(adj, adj.CuryDocBal,
							new PXSetPropertyException(Messages.PrepaymentNotPayedFull, adj.AdjdRefNbr));

						throw new PXRowPersistingException(typeof(APPayment.hold).Name, doc.Hold, Messages.PrepaymentNotPayedFull, adj.AdjdRefNbr);
					}
				}
			}
		}
