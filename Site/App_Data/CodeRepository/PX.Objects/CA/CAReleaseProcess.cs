...
namespace PX.Objects.CA
{
	...
	public class CAReleaseProcess : PXGraph<CAReleaseProcess>
	{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo, CAAdj> res in resultset)
				{
					...
					if (shouldSegregate)
					{
						...
						var batchFinPeriodID = FinPeriodRepository.GetPeriodIDFromDate(catran.TranDate, organizationID);

						SegregateBatch(je, batchlist, branchID, catran.CuryID, catran.TranDate, batchFinPeriodID, catran.TranDesc, info);
					}
		...
		public virtual void ReleaseDeposit(JournalEntry je, ref List<Batch> batchlist, CADeposit doc)
		{
			...
			foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo, CADepositDetail, CADepositEntry.ARPaymentUpdate, CADepositEntry.APPaymentUpdate> res in resultset)
			{
				...
				if (!string.IsNullOrEmpty(detail.OrigRefNbr)
					...
					&& IsVoidedEntryWithoutPair(detail, select))
				{
		...
		private bool IsVoidedEntryWithoutPair(CADepositDetail detail, PXSelectBase<CATran> select)
		{
			...
			select.WhereAnd<Where<CADepositDetail.origModule, Equal<BatchModule.moduleAR>,
			And<CADepositDetail.origDocType, Equal<Required<CADepositDetail.origDocType>>,
				And<CADepositDetail.origRefNbr, Equal<Required<CADepositDetail.origRefNbr>>>>>>();

			if (select.Select(detail.TranType, detail.RefNbr,
				ARPaymentType.GetVoidingARDocType(detail.OrigDocType), detail.OrigRefNbr).Count == 1)
			{
				return false;
			}
