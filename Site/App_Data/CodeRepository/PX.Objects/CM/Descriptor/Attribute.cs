...
namespace PX.Objects.CM
{
	...
	public static class PaymentEntry
	{
		...
		{
			CurrencyInfo to_info = currencyinfoselect.Select(toCuryInfoID);
			CurrencyInfo from_originfo = currencyinfoselect.Select(fromOrigInfoID);
			...
			{
				CurrencyInfo from_info = currencyinfoselect.Select(fromCuryInfoID);
			...
			where TAdjustment : class, IBqlTable, IAdjustment
		{
			...
			if (curyAdjAmt != null)
			{
				...
				adj.DocBal = adj.CuryDocBal == 0m ? 0m : adj.DocBal - to_adjamt;
			}
			...
			if (curyAdjWhTaxAmt != null)
			{
				...
				adj.CuryWhTaxBal -= Math.Abs(decimalValue);
				adj.WhTaxBal = adj.CuryWhTaxBal == 0m ? 0m : adj.WhTaxBal - Math.Abs(to_adjwhtaxamt);

				if (adjustBalanceByExtraAmounts)
				{
					adj.CuryDocBal -= decimalValue;
					adj.DocBal = adj.CuryDocBal == 0m ? 0m : adj.DocBal - to_adjwhtaxamt;
				}
			}
			...
			if (curyAdjDiscAmt != null)
			{
				...
				{
					adj.CuryDocBal -= decimalValue;
					adj.DocBal = adj.CuryDocBal == 0m ? 0m : adj.DocBal - to_adjdiscamt;
				}
			}
		}
