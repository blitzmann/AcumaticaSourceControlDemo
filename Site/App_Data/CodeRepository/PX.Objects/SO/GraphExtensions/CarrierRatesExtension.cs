...
namespace PX.Objects.SO.GraphExtensions.CarrierRates
{
		...
		where TPrimary : class, IBqlTable, new()
	{
		...
		protected virtual CarrierRequest BuildRateRequest(Document doc)
		{
			...
			return GetCarrierRequest(doc, unit, methods, boxes);
		}
		...
		protected virtual CarrierBoxEx BuildCarrierBoxes(List<SOPackageInfoEx> list, IAddressBase origin, string method, string pluginUOM)
		{
		...
		{
			decimal weightInStandardUnit = CarrierMaint.ConvertGlobalUnits(Base, commonsetup.Current.WeightUOM, pluginUOM, boxInfo.GrossWeight ?? 0);
