...
namespace PX.Objects.SO
{
	...
	public class PickPackShip : WMSBase
	{
		...
		{
			if (HeaderView.Current.ScanState == ScanStates.PickConfirmation)
				SetScanState(ScanStates.PickConfirmation);
			else if (HeaderView.Current.ScanState == ScanStates.PackConfirmation)
				SetScanState(ScanStates.PackConfirmation);
		}
		...
		protected virtual void _(Events.RowSelected<SOShipment> e)
		{
			if (HeaderView.Current.ScanState == ScanStates.Wait)
			{
				...
				{
					if (Base.IsMobile)
						Clear(Msg.ShipmentIsConfirmed);
					else
						SetScanState(GetDefaultState(), Msg.ShipmentIsConfirmed);
				}
			}
		}
		#endregion
		...
		private SOShipment Shipment => Base.CurrentDocument.Current;
			...
			switch (state)
			{
				...
				case ScanStates.Location:
					if (IsLocationRequired(HeaderView.Current))
						...
						SetScanState(HeaderView.Current.Mode == Modes.Pick && ViseVersaFlow(HeaderView.Current) ? ScanStates.PickConfirmation : ScanStates.Item);
					break;
				...
				case ScanStates.Carrier:
					...
					break;
				case ScanStates.PickConfirmation:
					if (ExplicitLineConfirmation)
						...
						ConfirmPicked();
					break;
				case ScanStates.PackConfirmation:
					ConfirmPacked();
					break;
			}
		}
		...
		protected override bool CompleteRedirect(string command)
		{
			switch (command)
			{
				...
				case ScanRedirects.ModeShip:
					HeaderView.Current.CartLoaded = false;
					if (HeaderView.Current.RefNbr != null) UpdateRates();
					return true;
			}
		...
		protected override void ProcessLocationBarcode(string barcode)
		{
			...
			else
			{
				...
				SetScanState(ViseVersaFlow(HeaderView.Current) ? ScanStates.PickConfirmation : ScanStates.Item, Msg.LocationReady, location.LocationCD);
			}
		}
		...
		protected override void ProcessItemBarcode(string barcode)
		{
			...
			if (item == null)
			{
				if (HeaderView.Current.Mode == Modes.Pick
					...
					: Setup.Current.ShowPickTab != true)
				{
			...
			if (lsclass.LotSerTrack != INLotSerTrack.NotNumbered && (DefaultLotSerial == false || lsclass.LotSerAssign == INLotSerAssign.WhenUsed || lsclass.LotSerIssueMethod == INLotSerIssueMethod.UserEnterable)) // TODO: make same as for putaway
				...
				SetScanState(ViseVersaFlow(HeaderView.Current) ? ScanStates.Location : ScanStates.PickConfirmation);
			else if (HeaderView.Current.Mode == Modes.Pack)
				SetScanState(ScanStates.PackConfirmation);
		}
		...
		protected override void ProcessLotSerialBarcode(string barcode)
		{
			...
			if (HeaderView.Current.Mode == Modes.Pick)
			{
				if (HeaderView.Current.LotSerTrackExpiration == true && HeaderView.Current.LotSerAssign == INLotSerAssign.WhenUsed && Picked.SelectMain().Any(t => t.LotSerialNbr == barcode && t.PickedQty == 0)) // TODO: make same as for Receive
		...
		private WMSFlowStatus ConfirmPickedNoCart()
		{
		...
		private WMSFlowStatus ConfirmPickedInCart()
		{
			WMSFlowStatus Implementation()
			{
				Header header = HeaderView.Current;
		...
		private WMSFlowStatus ConfirmPickedOutCart()
		{
			WMSFlowStatus Implementation()
			{
				Header header = HeaderView.Current;
		...
		#region Cart sync
		private WMSFlowStatus SyncWithCart(Header header, SOShipLineSplit pickedSplit, decimal qty)
		{
		...
		private WMSFlowStatus EnsureShipmentCartSplitLink(SOShipLineSplit soSplit, INCartSplit cartSplit, decimal deltaQty)
		{
			var allLinks =
		...
		private WMSFlowStatus ConfirmPacked()
		{
			WMSFlowStatus Implementation()
			{
				Header header = HeaderView.Current;
		...
		private bool PackSplit(SOShipLineSplit split, SOPackageDetailEx package, decimal qty)
		{
			if (NoPick == false)
		...
		private decimal AutoCalculateBoxWeightBasedOnItems(SOPackageDetailEx package)
		{
		...
		private bool SkipBoxWeightInput()
		{
		...
		private void ConfirmShipment(bool confirmAll)
		{
			...
			WaitFor(delegate
			{
				...
				shipmentEntry.Document.Current =
					PXSelect<SOShipment, Where<SOShipment.shipmentNbr, Equal<Required<SOShipment.shipmentNbr>>>>
					.Select(shipmentEntry, shipmentNbr);
				...
				if (!confirmAll && (setup.ShowPickTab == true || setup.ShowPackTab == true))
				{
					...
					foreach (SOShipLine line in lines.Select())
					{
						...
						foreach (SOShipLineSplit split in splits.Select())
						{
							...
							decimal newQty = setup.ShowPickTab == true ? splits.Current.PickedQty ?? 0 : Math.Max(splits.Current.PickedQty ?? 0, splits.Current.PackedQty ?? 0);
		...
		private bool IsSelectedSplit(SOShipLineSplit split, Header header)
		{
			return
		...
		private decimal GetCartQty(SOShipLineSplit sosplit) => CartSplitLinks.SelectMain().Where(link => SOShipmentSplitToCartSplitLink.FK.ShipmentSplitLine.Match(Base, sosplit, link)).Sum(_ => _.Qty ?? 0);
		private decimal GetOverallCartQty(SOShipLineSplit sosplit) => AllCartSplitLinks.SelectMain().Where(link => SOShipmentSplitToCartSplitLink.FK.ShipmentSplitLine.Match(Base, sosplit, link)).Sum(_ => _.Qty ?? 0);

		protected override bool IsLocationRequired(Header header) => base.IsLocationRequired(header) && DefaultLocation == false;

		private bool DefaultLocation => UserSetup.For(Base).DefaultLocationFromShipment == true;
		private bool DefaultLotSerial => UserSetup.For(Base).DefaultLotSerialFromShipment == true;
		private bool NoPick => Setup.Current.ShowPickTab == false;
		private bool CanPick => HeaderView.Current.Mode == Modes.Pick && (Picked.SelectMain().Any(s => s.PickedQty < s.Qty));
		private bool CanPack => HeaderView.Current.Mode == Modes.Pack && Picked.SelectMain().Any(s => Setup.Current.ShowPickTab == true ? s.PickedQty > 0 && s.PackedQty < s.PickedQty : s.PackedQty < s.Qty);
		private bool HasUnconfirmedBoxes => Base.Packages.SelectMain().Any(p => p.Confirmed == false);
		private bool HasSingleAutoPackage(string shipmentNbr, out SOPackageDetailEx package)
		{
		...
		private void EnsureAssignedSplitEditing(SOShipLineSplit split)
		{
		...
		protected virtual bool SetLotSerialNbrAndQty(Header header, SOShipLineSplit pickedSplit, decimal qty)
		{
			...
			{
				var existingSplit = pickedSplit.IsUnassigned == true
		...
		private bool ViseVersaFlow(Header header) => header.Mode == Modes.Pick && header.InvtMult == 1;
		...
		private bool PromptLocationForEveryLine => true; // TODO: move to a setting
		...
		public new abstract class Msg : WMSBase.Msg
		{
			...
			public const string InventoryMissingInShipment = "The inventory item {0} does not exist in the shipment.";
			public const string LocationMissingInShipment = "The location {0} does not exist in the shipment.";
			public const string LotSerialMissingInShipment = "The lot/serial number {0} is not present in the shipment.";

			public const string CarrierPrompt = "Select a carrier.";

			public const string BoxPrompt = "Scan the box barcode.";
			public const string BoxReady = "Box {0} is selected.";
			...
			public const string ShipmentReady = "Shipment {0} is loaded and ready to be processed.";
			public const string ShipmentNbrMissing = "Shipment {0} is not found.";
			public const string ShipmentInvalid = "Shipment {0} has status invalid for processing.";
			public const string ShipmentInvalidSite = "Shipment {0} has the warehouse that differs from the warehouse assigned to the selected cart.";
			public const string ShipmentCannotBePacked = "The {0} shipment cannot be processed in the Pack mode because the shipment has multiple assigned packages.";
			public const string ShipmentPicked = "Shipment {0} is picked.";
			public const string ShipmentPacked = "Shipment {0} is packed.";
