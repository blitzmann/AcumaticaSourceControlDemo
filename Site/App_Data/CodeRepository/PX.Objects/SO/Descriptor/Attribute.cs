...
namespace PX.Objects.SO
{
		...
		public override void Plan_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (e.Operation == PXDBOperation.Update)
			{
				...
				if (row != null && order != null
					...
					&& PXLongOperation.GetCustomInfo(sender.Graph.UID, PXProcessing.ProcessingKey, out processingList) != null
					&& processingList != null
					&& processingList.Any(x => ((SOOrder)x).NoteID == row.RefNoteID))
				{
					e.Cancel = true;
				}
			}
		...
		private class ReturnRecord
		{
			...
			public static ReturnRecord FromSOLine(SOLine l)
			{
				return new ReturnRecord
				{
					...
					UOM = l.UOM,
					Qty = (l.RequireShipping == true && l.Completed == true) ? (l.ShippedQty ?? 0) : (l.OrderQty ?? 0)
				};
			}
	...
	public class SOLotSerialNbrAttribute : INLotSerialNbrAttribute
	{
		...
		public SOLotSerialNbrAttribute(Type InventoryType, Type SubItemType, Type LocationType)
		{
			...
			Where<INLotSerialStatus.inventoryID, Equal<Optional<BqlPlaceholder1>>,
				And<INLotSerialStatus.subItemID, Equal<Optional<BqlPlaceholder2>>,
				And2<Where<INLotSerialStatus.locationID, Equal<Optional<BqlPlaceholder3>>,
					Or<Optional<BqlPlaceholder3>, IsNull>>, 
				And<INLotSerialStatus.qtyOnHand, Greater<decimal0>>>>>>>
				.Replace<BqlPlaceholder1>(InventoryType)
				.Replace<BqlPlaceholder2>(SubItemType)
				.Replace<BqlPlaceholder3>(LocationType).ToType();
		...
		public class SOAllocationLotSerialNbrAttribute : SOLotSerialNbrAttribute
		{
			public SOAllocationLotSerialNbrAttribute(Type InventoryType, Type SubItemType, Type SiteType, Type LocationType)
			{
				...
				Where<INLotSerialStatus.inventoryID, Equal<Optional<BqlPlaceholder1>>,
					And<INLotSerialStatus.subItemID, Equal<Optional<BqlPlaceholder2>>,
					And2<Where<INLotSerialStatus.siteID, Equal<Optional<BqlPlaceholder3>>,
						Or<Optional<BqlPlaceholder3>, IsNull>>,
					And2 <Where<INLotSerialStatus.locationID, Equal<Optional<BqlPlaceholder4>>,
						Or<Optional<BqlPlaceholder4>, IsNull>>, 
					And<INLotSerialStatus.qtyOnHand, Greater<decimal0>>>>>>>>
					.Replace<BqlPlaceholder1>(InventoryType)
					.Replace<BqlPlaceholder2>(SubItemType)
					.Replace<BqlPlaceholder3>(SiteType)
					.Replace<BqlPlaceholder4>(LocationType).ToType();
