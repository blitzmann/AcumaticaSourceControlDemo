...
namespace PX.Objects.SO
{
	...
	public class SOOrderEntry : PXGraph<SOOrderEntry, SOOrder>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess, IGraphWithInitialization
	{
		...
		public virtual IEnumerable POSupply()
		{
			...
			foreach (SOLineSplit splitFromCache in PXSelect<SOLineSplit, Where<SOLineSplit.orderType, Equal<Required<SOLineSplit.orderType>>, And<SOLineSplit.orderNbr, Equal<Required<SOLineSplit.orderNbr>>>>>.Select(this, soline.OrderType, soline.OrderNbr).Where(x => ((SOLineSplit)x).PONbr != null))
			{
				...
				if (existingResult.POOrderNbr != null && splitFromCache.POType == existingResult.POOrderType && splitFromCache.PONbr == existingResult.POOrderNbr && splitFromCache.POLineNbr == existingResult.POLineNbr)
				{
					if (splitFromCache.LineNbr == soline.LineNbr)
					{
			...
			foreach (POSupplyResult res in mergedResults)
			{
				...
				if (!linkedWithCurrentDocument 
					&& (poorder.Hold == true && !SOPOLinkShowDocumentsOnHold || (allSplitsCompleted || soline.Completed == true)))
						continue;

				if (linkedWithCurrentDocument
					|| supply.OrderType != PO.POOrderType.DropShip && supply.Completed == false && supply.Cancelled == false && supply.BaseOpenQty - demandQty > 0m
					|| supply.OrderType == PO.POOrderType.DropShip && !linkedWithForeignDocument && 
						(supply.Completed == false && supply.Cancelled == false && supply.BaseOrderQty >= 0m || supply.BaseReceivedQty > 0m))
				{
					...
					supply.SOOrderSplitLineNbr = linkedWithCurrentDocument ? linkWithCurrentDocument.SplitLineNbr : null;
					supply.VendorRefNbr = poorder.VendorRefNbr;
					supply.DemandQty = demandQty;

					POLine3 cached = posupply.Locate(supply);

					///posupply cache should be kept in an up-to-date state. 
					///Cached records will be updated with values from the current SOLine.
					///This cache is lately used in <see cref = "LinkSupplyDemand()"/>
					if (cached == null || (cached.Selected == supply.Selected && cached.Selected != true && cached.SOOrderLineNbr != supply.SOOrderLineNbr))
					{
		...
		public virtual void CreatePaymentProc(SOOrder order, out PXGraph target, string paymentType = ARPaymentType.Payment)
		{
			...
			if (order.IsCCCaptured == true || order.IsCCAuthorized == true)
			{
				...
				if (order.IsCCCaptured == true)
				{
					...
					if (!ps.isOpenForReview && payment.Released == false && arsetup.Current.IntegratedCCProcessing == true)
					{
		...
		public class PaymentProfileHostedForm : Extensions.PaymentProfile.PaymentProfileGraph<SOOrderEntry,SOOrder>
		{
			...
			protected override void RowSelected(Events.RowSelected<SOOrder> e)
			{
				...
				if (row.CreatePMInstance.GetValueOrDefault())
				{
					...
					this.RefreshSyncPaymentAction(enable && base.CustomerPaymentMethod.Current?.CustomerCCPID != null);
				}
				else
				{
					this.RefreshCreatePaymentAction(false, false);
					this.RefreshSyncPaymentAction(false);
				}
		...
		protected virtual void SOOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				...
				{
					paymentState = new PaymentState(ccProcTran.Select());
					isCCRefunded = paymentState.isCCRefunded;
					isCCCaptured = paymentState.isCCCaptured;
					isCCPreAuthorized = paymentState.isCCPreAuthorized;
				}
		...
		protected virtual void SOLine_InventoryID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			using(GetPriceCalculationScope().AppendContext<SOLine.inventoryID>())
		...
		protected virtual bool IsManualPriceFlagNeeded(PXCache sender, SOLine row)
		{
			if (row != null && row.ManualPrice != true && sender.Graph.IsImportFromExcel)
			{
				...
				//TODO: curyLineAmt field should be replaced with curyExtPrice in 2019R1
				object curyLineAmt = sender.GetValuePending<SOLine.curyLineAmt>(row);
				...
				if (((curyUnitPrice != PXCache.NotSetValue && curyUnitPrice != null && Decimal.TryParse(curyUnitPrice.ToString(), out price))
					|| (curyLineAmt != PXCache.NotSetValue && curyLineAmt != null && Decimal.TryParse(curyLineAmt.ToString(), out price)))
		...
		protected virtual void SOLine_CuryLineAmt_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			//TODO: this check should be performed in CuryExtPrice_FieldVerifying in 2019R1
			...
			public void Authorize(PXCache sender, PXRowPersistedEventArgs e)
			{
				if (e.TranStatus == PXTranStatus.Open)
				{
					...
					if (iOrder != null)
					{
						...
						if (ccProcGraph.RecordAuthorization(iOrder, tranRecord))
						{
							iOrder.CCAuthTranNbr = tranRecord.InnerTranId;
						}
					}
				}
			}
		...
		public virtual UpdateIfFieldsChangedScope GetPriceCalculationScope()
			=> new PriceCalculationScope<ARSalesPrice, ARSalesPrice.recordID>();
	...
	public class PaymentTransaction : PaymentTransactionGraph<SOOrderEntry, SOOrder>
	{
		...
		{
			base.MapViews(graph);
			this.PaymentTransaction = new PXSelectExtension<Extensions.PaymentTransaction.PaymentTransactionDetail>(Base.ccProcTran);
		}
		...
		protected override void RowSelected(Events.RowSelected<SOOrder> e)
		{
			...
			doc.PCResponseReasonText = string.Empty;
			if (paymentState.lastTran != null && paymentState.lastTran is PaymentTransactionDetail)
			{
				doc.PCResponseReasonText = ((PaymentTransactionDetail)paymentState.lastTran).PCResponseReasonText;
			}

			if (paymentState.isCCPreAuthorized)
			{
				doc.CCAuthTranNbr = paymentState.lastTran.TranNbr;
				doc.PreAuthTranNumber = paymentState.lastTran.PCTranNumber;
			}
			if (paymentState.isCCCaptured)
			{
				doc.CCAuthTranNbr = paymentState.lastTran.TranNbr;
				doc.CaptureTranNumber = paymentState.lastTran.PCTranNumber;
			}
			...
			{ 
				...
				bool canAuthorize = !(paymentState.isCCPreAuthorized || paymentState.isCCCaptured) && isNotNullAmt;
				bool canCapture = !(paymentState.isCCCaptured) && isNotNullAmt;
				bool canVoid = (paymentState.isCCCaptured || paymentState.isCCPreAuthorized);
				bool canRefund = paymentState.isCCRefunded;
				bool isCashReturn = doc.ARDocType == ARDocType.CashReturn;
				...
				cache.AllowDelete = !CCProcTranHelper.HasSuccessfulCCTrans(Base.ccProcTran) || paymentState.isNone || paymentState.isCCVoided;
				if (paymentState.isOpenForReview)
				{
		...
		public static void UpdateSOOrderState(IBqlTable aDoc, CCTranType aLastOperation, bool success)
		{
			UpdateDocState(aDoc as SOOrder, aLastOperation);
		}
		...
		public static void UpdateDocState(SOOrder olddoc, CCTranType lastOperation)
		{
			...
			CCProcTranHelper.CCTransState ccState = CCProcTranHelper.UpdateCCPaymentState<SOOrder>(doc, graph.ccProcTran.Select());

			CCProcTran voidedTran = PXSelect<CCProcTran,
					Where<CCProcTran.origRefNbr, Equal<Current<SOOrder.orderNbr>>,
						And<CCProcTran.origDocType, Equal<Current<SOOrder.orderType>>,
						And<CCProcTran.tranType, Equal<CCTranTypeCode.voidTran>,
						And<CCProcTran.pCTranNumber, Equal<Current<SOOrder.extRefNbr>>>>>>>.Select(graph);
			...
			try
			{
				if (graph.soordertype.Current.CanHaveApplications)
				{
					...
					if ((ccState.PaymentState.isCCVoided || ccState.PaymentState.isCCRefunded)
						&& graph.arsetup.Current.IntegratedCCProcessing == true)
					{
						...
						ARPayment payment = PXSelect<ARPayment, Where<ARPayment.docType, Equal<Required<ARPayment.docType>>,
							And<ARPayment.refNbr, Equal<Required<ARPayment.refNbr>>>>>.Select(docgraph, ccState.PaymentState.lastTran.DocType, ccState.PaymentState.lastTran.RefNbr);
						...
						{
							message = new PXSetPropertyException(Messages.CouldNotVoidCCTranPayment, PXErrorLevel.Warning, ccState.PaymentState.lastTran.RefNbr);
						}
					...
					if (ccState.PaymentState.isCCCaptured)
					{
						graph.Save.Press();
						PXGraph target;
