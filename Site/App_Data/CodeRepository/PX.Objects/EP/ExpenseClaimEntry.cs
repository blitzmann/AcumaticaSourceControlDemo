...
namespace PX.Objects.EP
{
	public class ExpenseClaimEntry : PXGraph<ExpenseClaimEntry, EPExpenseClaim>, PXImportAttribute.IPXPrepareItems
	{
		...
		{
			var item = PXSelectorAttribute.Select<InventoryItem.inventoryID>(e.Cache, e.Row) as InventoryItem;
			decimal curyStdCost;
			if (item != null)
			{
				var cost = e.Row.ExpenseDate >= item.StdCostDate ? item.StdCost.GetValueOrDefault() : item.LastStdCost.GetValueOrDefault();
				PXCurrencyAttribute.CuryConvCury<EPExpenseClaimDetails.curyInfoID>(e.Cache, e.Row, cost, out curyStdCost, true);
			}
			else
				curyStdCost = 0m;
			e.NewValue = curyStdCost;
		}
