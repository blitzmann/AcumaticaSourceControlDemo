...
namespace PX.Objects.IN
{
    ...
    public class INReleaseProcess : PXGraph<INReleaseProcess>
    {
            ...
            if (trancost.COGSAcctID != null || intran.AcctID != null)
            {
                ...
                else
                {
					...
					if (intran.BranchID != site.BranchID)
					    FinPeriodIDAttribute.SetPeriodsByMaster<GLTran.finPeriodID>(je.GLTranModuleBatNbr.Cache, tran, intran.TranPeriodID);
				}
        ...
        public virtual void UpdateSplitDestinationLocation(INTranSplit split, int? value)
        {
            ...
            {
				INLocation whseFromLoc = INLocation.PK.Find(this, split.LocationID);
				INLocation whseToLoc = INLocation.PK.Find(this, split.ToLocationID);
		...
		public virtual void UpdateINRegisterReleased(INRegister doc)
		{
			...
			bool setReleasedFlag = PXDatabase.Update<INRegister>(
				new PXDataFieldAssign<INRegister.released>(true),
				new PXDataFieldRestrict<INRegister.docType>(doc.DocType),
				new PXDataFieldRestrict<INRegister.refNbr>(doc.RefNbr),
				new PXDataFieldRestrict<INRegister.released>(false));
			if (!setReleasedFlag)
			{
...
namespace PX.Objects.IN.Overrides.INDocumentRelease
{
	...
	public partial class SiteStatus : INSiteStatus, IQtyAllocated
	{
		...
		[PXDBScalar(typeof(Search2<INItemClass.negQty, 
			...
			Where<InventoryItem.inventoryID, Equal<INSiteStatus.inventoryID>>>))]
    ...
    public partial class LotSerialStatus : INLotSerialStatus, IQtyAllocated
    {
        ...
        [PXDBScalar(typeof(Search2<INItemClass.negQty, 
	        InnerJoin<InventoryItem, 
		        On<InventoryItem.FK.ItemClass>>, 
	        Where<InventoryItem.inventoryID, Equal<INLotSerialStatus.inventoryID>>>))]
        public virtual bool? NegQty
        {
            get
            {
    ...
    public partial class ItemLotSerial : INItemLotSerial, IQtyAllocatedSeparateReceipts
    {
        ...
        [PXDBScalar(typeof(Search2<INItemClass.negQty, 
	        InnerJoin<InventoryItem, 
		        On<InventoryItem.FK.ItemClass>>, 
	        Where<InventoryItem.inventoryID, Equal<INItemLotSerial.inventoryID>>>))]
        public virtual bool? NegQty
        {
            get
            {
    ...
    public partial class OversoldCostStatus : INCostStatus
    {
        ...
        [PXDBDecimal(6)]
		[PXDefault(TypeCode.Decimal, "0.0", typeof(Search2<INItemSiteSettings.negativeCost,
	        InnerJoin<INLocation,
					On<INLocation.siteID, Equal<INItemSiteSettings.siteID>>>,
				Where<INItemSiteSettings.inventoryID, Equal<Current<OversoldCostStatus.inventoryID>>,
					And<Where<INItemSiteSettings.siteID, Equal<Current<OversoldCostStatus.costSiteID>>,
    ...
    public class SiteStatusAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns)
        {
			...
			if (sender.GetStatus(row) == PXEntryStatus.Inserted && IsZero((SiteStatus)row) 
				&& ((SiteStatus)row).QtyINReplaned == 0m) //QtyINReplaned should be added to IStatus interface
			{
    ...
    public class TransitSiteStatusAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns)
        {
			...
			if (sender.GetStatus(row) == PXEntryStatus.Inserted && IsZero((TransitSiteStatus)row)
				&& ((TransitSiteStatus)row).QtyINReplaned == 0m) //QtyINReplaned should be added to IStatus interface
			{
				sender.SetStatus(row, PXEntryStatus.InsertedDeleted);
				return false;
			}
    ...
    public class ItemLotSerialAccumulatorAttribute : StatusAccumulatorAttribute
    {
		...
		{
			PXCache cache;
			graph.Caches.TryGetValue(typeof(ItemLotSerial), out cache);
			if (cache != null)
			{
    ...
    public class SiteLotSerialAccumulatorAttribute : StatusAccumulatorAttribute
    {
		...
		{
			PXCache cache;
			graph.Caches.TryGetValue(typeof(SiteLotSerial), out cache);
			if (cache != null)
			{
