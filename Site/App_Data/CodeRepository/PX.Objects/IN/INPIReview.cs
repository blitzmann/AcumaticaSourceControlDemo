...
namespace PX.Objects.IN
{
	...
	public class INPIEntry : INPIController, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
			...
			bool forseDebitLinesRecalculation = false)
		{
			...
			foreach (var debitLine in debitLines)
			{
				...
				additionalDebitExtCost += (decimal)debitLine.ExtVarCost;
		...
		private decimal GetAdditionalUnitCost(INPIDetail detail, decimal additionalDebitQty, decimal additionalDebitExtCost)
		{
			decimal avgDebitLinesUnitCost = additionalDebitQty != 0m ? PXDBCurrencyAttribute.BaseRound(this, additionalDebitExtCost / additionalDebitQty) : 0m;
		...
		protected virtual List<ProjectedTranRec> RecalcDemandCost(bool adjustmentCreation = false, bool forseDebitLinesRecalculation = false)
		{
			...
			var projectedTrans = new List<ProjectedTranRec>();
			var relatedDetailGroups = PIDetail.Select()
				.AsEnumerable()
				.RowCast<INPIDetail>()
		...
		protected virtual void RecalcTotals()
		{
			...
			foreach (INPIDetail detail in PIDetail.Select())
			{
	...
	public class INPIReview : INPIEntry  // (= PIEntry + some extended functionality)
	{
		...
		protected virtual IEnumerable SetNotEnteredToZero(PXAdapter adapter)
		{
			...
			foreach (INPIDetail detail in PIDetail.Select())
			{
				if (detail.Status != INPIDetStatus.NotEntered || !AreKeysFieldsEntered(detail))
					continue;

				PIDetail.Cache.SetValueExt<INPIDetail.physicalQty>(detail, 0m);
				PIDetail.Cache.MarkUpdated(detail);
			}
			DisableCostCalculation = false;

			RecalcDemandCost();
			RecalcTotals();
			PIDetail.Cache.IsDirty = true;
		...
		protected virtual IEnumerable SetNotEnteredToSkipped(PXAdapter adapter)
		{
			...
			foreach (INPIDetail detail in PIDetail.Select())
			{
				...
				PIDetail.Cache.SetValue<INPIDetail.status>(detail, INPIDetStatus.Skipped);
				PIDetail.Cache.MarkUpdated(detail);
			}
