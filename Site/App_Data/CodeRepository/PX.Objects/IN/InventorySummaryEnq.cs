...
namespace PX.Objects.IN
{
	...
	public partial class InventorySummaryEnquiryResult : PX.Data.IBqlTable, IStatus, ICostStatus 
	{
		...
		public abstract class inventoryID : PX.Data.BQL.BqlInt.Field<inventoryID> { }
		protected Int32? _InventoryID;
		[Inventory(Visibility = PXUIVisibility.SelectorVisible, DisplayName = "Inventory ID", IsKey = true, Visible = false)]
		public virtual Int32? InventoryID
		{
			get
			{
	...
	{				
		...
		protected virtual IEnumerable iSERecords()
		{
			...
			if (ISERecords.Cache.Cached.Count() == 0 || ISERecords.Cache.Cached.RowCast<InventorySummaryEnquiryResult>().First().ControlTimetamp != controlTimeStamp)
		...
		{
			var resultset = new List<InventorySummaryEnquiryResult>();
			...
			foreach (PXResult<INLotSerialStatus, INLocation, InventoryItem> res in cmd_lss.Select())
			{
				...
				{
					if (!ret.IsZero()) resultset.Add(ret);
				}
			}
		...
		private String ControlTimeStamp
		{
			get
			{
				...
				if (defs == null)
				{
					...
					defs = PXDatabase.GetSlot<Definition, InventorySummaryEnq>(nameof(InventorySummaryEnq) + "$ControlTimeStampDefinition", this,
		...
		public class Definition : IPrefetchable<InventorySummaryEnq>
		{
			public String TimeStamp { get; private set; }

			public void Prefetch(InventorySummaryEnq graph)
			{
				graph.ISERecords.Cache.Clear();
